const teams = {
  ...
  Clippers: [
    ...
    { name: "Amir Coffey", position: "SG", careerPoints: 1282 },
...
// Function to display career points
function displayCareerPoints(player, careerPoints) {
  player.setAttribute("title", `Career Points: ${careerPoints}`);
}

// Function to display players
function displayPlayers(filter = "") {
  const playerLibrary = document.getElementById("player-library");
  playerLibrary.innerHTML = ""; // Clear existing content

  for (const team in teams) {
    if (team.toLowerCase().includes(filter.toLowerCase())) {
      teams[team].forEach((player) => {
        const image = createElement("img", "player-image");
        const playerName = player.name
          .replace(/'/g, "_")
          .replace(/ /g, "_")
          .toLowerCase();
        image.src = `images/${team.toLowerCase()}/${
          player.position
        }/${playerName}.png`;
        image.alt = player.name;

        displayCareerPoints(image, player.careerPoints);

        playerLibrary.appendChild(image);
      });
    } else {
      teams[team].forEach((player) => {
        if (
...
document.getElementById("search-input").addEventListener("input", function () {
  ...
  displayPlayers(searchText);
});

// Call the function to display players initially
displayPlayers();
